...
*   **Гипотеза: Зависимость цены от площади ($H_0$: связь есть)**
    *   Корреляционные тесты Пирсона и Спирмена показали $p\text{-value} \approx 0$.
    *   **Вывод:** Первая гипотеза уверенно отвергается. Существует сильная, статистически значимая положительная связь (как линейная, так и монотонная) между жилой площадью (`sqft_living`) и стоимостью дома.
